...
Where Grafana spends time answering dashboard queries, split into the whole
request and the part spent inside the data source plugin, per plugin. It is the
Grafana-side complement of the DB Query Performance dashboard: Postgres only
records statements that finish, while Grafana records every request, including
the ones that were cancelled because the client gave up.
...
Grafana exposes Prometheus metrics on `grafana:3000/metrics` by default. The
`grafana` job in [`prometheus/prometheus.yml`](../prometheus/prometheus.yml)
scrapes it every 30 s. The dashboard reads two histograms:
...
type (`postgres`, `prometheus`), not a dashboard, panel or statement.
...
first scrape lands. Grafana itself picks the dashboard file up within 10 s, no
restart needed.
...
- A failing SQL statement and a statement cancelled because the client went
  away are both recorded as HTTP 400 in the request panels, with the plugin
  status still `ok`. Verified on Grafana 10.2.0: `GF_DATAPROXY_TIMEOUT` does
  not cancel SQL queries, and a client disconnect cancels the statement in
  Postgres and is counted as a 400.
